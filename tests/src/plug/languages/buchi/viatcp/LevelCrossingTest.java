...
public class LevelCrossingTest {
	...
	// String ltl = "vivacite = [] (|train.state == STATE_TRAIN_CLOSE| -> <>
	// |train.state == STATE_TRAIN_PASSING|)";
	// productSemantics.verify("", ltl, true);
	// }
	...
	// AbstractExplorer explorer = new BFSExplorer(kripkeRuntime, new
	// SimpleStateSpaceManager<>());
	//
	...
	// ConfigurationTreeGenerator.generateTree((Configuration) ev.getFinalState());
	...
	// explorer.execute();
	//
	// assertTrue("The model has " + nbDeadlocks[0] + " deadlocks.",
	// deadLockFree[0]);
	// }
	...
	// String ltl = "exclusion = ![]!(|train.state == STATE_TRAIN_DONE| &&
	///////////////////////////////////////////////// |roadSign.state ==
	...
	// String ltl = "fairness = ! ([] (|{sys}1:flags[0] = true| -> <> |{Alice}1@CS|)
	// && (|{sys}1:flags[1] = true| -> <> |{Bob}1@CS|) )";
